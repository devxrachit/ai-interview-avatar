...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (status === "authenticated") {
      analyticsApi.overview().then((r) => setOverview(r.data)).catch(() => {}).finally(() => setLoading(false));
    }
  }, [status]);
          ...
          {!overview?.recent_sessions?.length ? (
            ...
            <div className="space-y-3">
              {overview.recent_sessions.map((s) => (
                <div key={s.id} className="bg-[#12121a] border border-[#2a2a3e] hover:border-indigo-500/40 rounded-xl p-5 flex items-center justify-between transition-all">
                  <div className="flex items-center gap-4">
